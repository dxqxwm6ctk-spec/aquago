...
import 'package:customer_app/core/device_label.dart';
...
void main() {
  ...
  test('الصيغة القديمة كانت تُرفض فعلاً عند تكوين الطلب', () async {
    await expectLater(
      sendWithDeviceHeader('AquaGo Android · Google sdk_gphone16k_x86_64 (SDK 37)'),
  ...
  group('asciiSafe — على مدخلات الجهاز', () {
    test('التسمية الكاملة كما تُبنى على أندرويد تصلح ترويسةً بعد التعقيم',
        () async {
      final sanitized =
          asciiSafe('AquaGo Android · Google sdk_gphone16k_x86_64 (SDK 37)');
    ...
    test('اسم جهاز بغير اللاتينية لا يكسر الترويسة', () async {
      ...
      await expectLater(
        sendWithDeviceHeader(asciiSafe('AquaGo Android - هاتف سامسونج (SDK 37)')),
        completes,
      );
    });

    test('المحارف اللاتينية والأرقام والعلامات تمرّ بلا تشويه', () {
      const plain = 'AquaGo Android - Google sdk_gphone16k_x86_64 (SDK 37)';
